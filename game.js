  ...
  if (agreementstage == 0 || agreementstage == 2)
  {
    ...
    if (islegal (board, getx (x, size), getx (y, size), goes % 2 + 1))
    {
      ...
      else
      {
        ...
        passes = 0;
	socket.emit('move', getx (x, size) + getx (y, size) * 19);
...
{
	console.log("someone else made a move!");	
	if (lastmoves [lastmoves.length - 1] != data)
	{
		goes++;
      		board = makemove (board, data % 19, (data - (data % 19)) / 19, goes % 2 + 1);		
		lastmoves[goes] = data
		passes = 0;
		while (boards.length > goes)
		{
